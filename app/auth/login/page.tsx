      ...
      <div className="w-full max-w-sm">
        <div className="text-center mb-8">
          <p className="text-xs uppercase tracking-widest mb-2"
            style={{ color: "#C4954A", fontFamily: "var(--font-assistant)" }}>ברוכים השבים</p>
        ...
        <form onSubmit={handleLogin} className="flex flex-col gap-4">
          ...
          <div className="flex flex-col gap-1.5">
            <div className="flex justify-between items-center">
              ...
              <Link href="/auth/forgot-password" className="text-xs hover:opacity-100"
                style={{ color: "#C4954A", opacity: 0.7, fontFamily: "var(--font-assistant)" }}>
                שכחתי סיסמה
              </Link>
            </div>
          ...
          <div className="flex items-center gap-3">
            <div className="flex-1 h-px" style={{ backgroundColor: "rgba(196,149,74,0.2)" }} />
            <span className="text-xs opacity-40" style={{ color: "#F7F2E8", fontFamily: "var(--font-assistant)" }}>או</span>
            <div className="flex-1 h-px" style={{ backgroundColor: "rgba(196,149,74,0.2)" }} />
          </div>
          <button type="button" onClick={loginWithGoogle}
            className="w-full py-3 text-sm flex items-center justify-center gap-2 transition-opacity hover:opacity-80"
        ...
        <p className="text-center mt-6 text-sm opacity-60"
          ...
          אין לכם חשבון עדיין?{" "}
          <Link href={`/auth/sign-up?next=${next}`} style={{ color: "#C4954A" }} className="underline">הרשמה</Link>
        </p>
      </div>
    </div>
  );
