...
import { Map, Waves, TreePine, Sun, Mountain, Droplets, Droplet, MessageCircle, Moon, CheckCircle, LayoutList, type LucideIcon } from "lucide-react";
  ...
  return (
    <main style={{ minHeight: "100vh", display: "flex", flexDirection: "column" }}>
      ...
      <section
        ...
        style={{ minHeight: "300px", paddingTop: "80px", paddingBottom: "48px", backgroundColor: "#1C1814" }}
      >
        <div className="relative z-10 max-w-3xl mx-auto px-6 flex flex-col items-center gap-4">
          ...
          <h1
            ...
            style={{ fontFamily: "var(--font-cormorant)", fontSize: "clamp(2.4rem, 6vw, 4.5rem)", color: "#FAFAF6", lineHeight: 1.05 }}
          >
            כל ישראל —<br />
          ...
          <p
            ...
            style={{ fontFamily: "var(--font-assistant)", fontSize: "clamp(0.9rem, 1.3vw, 1.1rem)", color: "#FAFAF6", opacity: 0.65, lineHeight: 1.75 }}
          >
      ...
      >
        <div className="max-w-7xl mx-auto px-4 md:px-8 py-3 flex flex-col gap-3">
          ...
          {/* Top row: view toggle + region tabs */}
          <div className="flex items-center gap-3 flex-wrap">

            {/* Map / List toggle */}
            <div className="flex" style={{ border: "1px solid rgba(28,24,20,0.2)", borderRadius: 0 }}>
              <button
                onClick={() => setView("map")}
                className="flex items-center gap-1.5 px-4 py-2 text-xs transition-all"
                style={{
                  backgroundColor: view === "map" ? "#1C1814" : "transparent",
                  color: view === "map" ? "#FAFAF6" : "#6B5E4E",
                  fontFamily: "var(--font-assistant)",
                  letterSpacing: "0.08em",
                  fontWeight: view === "map" ? 600 : 400,
                }}
              >
                <Map size={13} strokeWidth={1.5} /> מפה
              </button>
              <div style={{ width: "1px", backgroundColor: "rgba(28,24,20,0.2)" }} />
              <button
                onClick={() => setView("list")}
                className="flex items-center gap-1.5 px-4 py-2 text-xs transition-all"
                style={{
                  backgroundColor: view === "list" ? "#1C1814" : "transparent",
                  color: view === "list" ? "#FAFAF6" : "#6B5E4E",
                  fontFamily: "var(--font-assistant)",
                  letterSpacing: "0.08em",
                  fontWeight: view === "list" ? 600 : 400,
                }}
              >
                <LayoutList size={13} strokeWidth={1.5} /> רשימה
              </button>
            </div>
            ...
            <div className="flex gap-1.5 flex-wrap">
              {REGION_TABS.map((tab) => (
                <button
                  ...
                  style={{
                    backgroundColor: activeRegion === tab.value ? "rgba(28,24,20,0.1)" : "transparent",
                    color: activeRegion === tab.value ? "#1C1814" : "#6B5E4E",
                    border: `1px solid ${activeRegion === tab.value ? "rgba(28,24,20,0.4)" : "rgba(28,24,20,0.15)"}`,
                    fontFamily: "var(--font-assistant)",
          ...
          {/* Bottom row: landscape + toggles (list view only) */}
          {view === "list" && (
            <div className="flex flex-wrap items-center gap-2">
              ...
              <div style={{ width: "1px", height: "18px", backgroundColor: "rgba(28,24,20,0.15)" }} />
              <button
              ...
              >
                <Moon size={12} strokeWidth={1.5} /> לינת לילה
              </button>
              <button
      ...
      {/* ── MAP VIEW ── */}
      {view === "map" && (
        <div style={{ flex: 1, minHeight: "calc(100vh - 290px)", display: "flex", flexDirection: "column" }}>
          <MapWrapper />
        </div>
      )}
      ...
      {/* ── LIST VIEW ── */}
      {view === "list" && (
        <section className="py-10 md:py-14 px-4 md:px-8 flex-1" style={{ backgroundColor: "#F0EDE4" }}>
          <div className="max-w-7xl mx-auto">
            ...
            {filtered.length === 0 ? (
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-5">
                {filtered.map((loc) => (
                  ...
                  >
                    {/* Image */}
                    <div className="relative overflow-hidden" style={{ aspectRatio: "16/9", backgroundColor: "#E8E0D4" }}>
                      ...
                      />
                      <div className="absolute inset-0" style={{ background: "linear-gradient(to top, rgba(15,13,10,0.6) 0%, rgba(15,13,10,0.05) 55%)" }} />
                      ...
                      <div className="absolute bottom-3 right-3 flex gap-1.5">
                        {loc.overnight && (
                          <span className="text-xs px-2 py-0.5" style={{ backgroundColor: "rgba(184,154,53,0.3)", color: "#B89A35", fontFamily: "var(--font-assistant)", backdropFilter: "blur(4px)" }}>לינה</span>
                        )}
                        {!loc.fee && (
                          <span className="text-xs px-2 py-0.5" style={{ backgroundColor: "rgba(28,24,20,0.5)", color: "#FAFAF6", fontFamily: "var(--font-assistant)" }}>חינמי</span>
                        )}
                        {loc.vehicle4x4 && (
                          <span className="text-xs px-2 py-0.5" style={{ backgroundColor: "rgba(74,106,74,0.4)", color: "#FAFAF6", fontFamily: "var(--font-assistant)" }}>4×4</span>
                        )}
                      </div>
                    </div>
                    ...
                    <div className="flex flex-col gap-2.5 p-4">
                      <div className="flex items-start justify-between gap-2">
                        <h3 className="font-light leading-tight" style={{ fontFamily: "var(--font-cormorant)", fontSize: "1.3rem", color: "#1C1814" }}>
                          {loc.nameHe}
                        </h3>
                        <span className="text-xs shrink-0 mt-0.5" style={{ color: "#B89A35", fontFamily: "var(--font-assistant)" }}>
                          {loc.regionHe}
                        </span>
                      </div>
                      ...
                      <p
                        ...
                        style={{
                          fontFamily: "var(--font-assistant)", color: "#6B5E4E", opacity: 0.85,
                      ...
                      {loc.amenities.length > 0 && (
                        <div className="flex gap-1.5 flex-wrap mt-1" style={{ color: "#6B5E4E" }}>
                          {loc.amenities.slice(0, 5).map((a) => (
                            <span key={a} title={a} className="text-sm" style={{ opacity: 0.6 }}>
                      ...
                      <div className="flex items-center justify-between pt-2 mt-auto" style={{ borderTop: "1px solid rgba(28,24,20,0.1)" }}>
                        <span className="text-xs" style={{ color: "#B89A35", fontFamily: "var(--font-assistant)", letterSpacing: "0.05em" }}>
                          {loc.recommendedTents.length} אוהלים מתאימים
                        </span>
                        <span className="text-xs opacity-0 group-hover:opacity-100 transition-opacity" style={{ color: "#1C1814", fontFamily: "var(--font-assistant)" }}>
                          ← פרטים
                        </span>
                      </div>
                    </div>
            ...
            <div className="mt-16 text-center">
              <p className="mb-5 font-light" style={{ fontFamily: "var(--font-cormorant)", fontSize: "clamp(1.5rem, 3vw, 2.2rem)", color: "#1C1814", opacity: 0.8 }}>
